import numpy as np
import functions as alg
import functions05 as alg_05
...
if __name__ == "__main__":
    ...
    x, values, runtimes, gd_xs = gradient_descent( alg.weird_func, initial_x, eps, maximum_iterations, backtracking_line_search )
    #    x, values, runtimes, gd_xs = msgd( alg.weird_func, initial_x, eps, maximum_iterations, backtracking_line_search,momentum=0.9 )
    ...
    func = lambda x, order: alg.quadratic( H, b, x, order )        
    # Find the (1e-4)-suboptimal solution
    eps = 1e-4
    maximum_iterations = 65536
...
#    initial_x = np.mat('1.0; 1.0;1.0')
    ...
    x, values, runtimes, gd_xs = gradient_descent( alg_05.fun_5_5, initial_x, eps, maximum_iterations, backtracking_line_search )
    print('Optimum of the fun_5_5 function found by GD with exact line search', x)
    
    initial_x = np.array([[4],[-4]])
    x, values, runtimes, gd_xs = gradient_descent( alg_05.fun_5_7, initial_x, eps, maximum_iterations, backtracking_line_search )
    print('Optimum of the fun_5_7 function found by GD with exact line search', x)
    

    initial_x = np.array([[0.8],[1]])
    x, values, runtimes, gd_xs= gradient_descent( alg_05.fun_5_9, initial_x, 1e-6, maximum_iterations, backtracking_line_search )
    print('Optimum of the fun_5_7 function found by GD with exact line search', x)
    

    x, values, runtimes, gd_xs = newton( alg_05.fun_5_9, initial_x, 1e-6, maximum_iterations, backtracking_line_search )
    print('Optimum of the fun_5_7 function found by GD with exact line search', x)
#  
