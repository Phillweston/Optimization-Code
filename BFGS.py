...
###############################################################################
def bfgs( func, initial_x, initial_inv_h, eps=1e-5, maximum_iterations=65536, linesearch=backtracking_line_search, *linesearch_args  ):
    """ 
    BFGS Algorithm
    ...
    *linesearch_args:   the extra arguments of linesearch routine
    ...
    while True:
        ...
        if condition >= 1e-9: 
            rho_k = 1 / condition
            inv_h = (np.identity(s_k.shape[0]) - rho_k * s_k.dot( y_k.T)).dot(inv_h).dot((np.identity(y_k.shape[0]) - rho_k * y_k.dot(s_k.T))) + rho_k * s_k.dot( s_k.T)
